...
export function useSplitMortgageCalculator() {
  ...
  const [person1Deposit, setPerson1Deposit] = useState<number>(baseCalculator.deposit / 2);
  const [person2Deposit, setPerson2Deposit] = useState<number>(baseCalculator.deposit / 2);
  const [person1RepaymentShare, setPerson1RepaymentShare] = useState<number>(0.5);
  ...
  if (person1Deposit < 0) validationErrors.person1Deposit = 'Person 1 deposit cannot be negative';
  if (person2Deposit < 0) validationErrors.person2Deposit = 'Person 2 deposit cannot be negative';
  ...
  if (person1RepaymentShare < 0) validationErrors.person1RepaymentShare = 'Repayment share cannot be negative';
  if (person1RepaymentShare > 1) validationErrors.person1RepaymentShare = 'Repayment share cannot exceed 100%';
  ...
  const results = useMemo(() => {
    ...
    if (selectedYear === 'first') {
      ...
      if (yearNumber <= termYears) {
        ...
        // Calculate total principal paid up to the selected year
        for (let i = 0; i < periodsToYear; i++) {
          const periodInterest: number = remainingBalance * r;
          const periodPrincipal: number = payment - periodInterest;
          remainingBalance -= periodPrincipal;
        ...
        const person1PrincipalContribution: number = totalPrincipalPaid * person1RepaymentShare;
        const person2PrincipalContribution: number = totalPrincipalPaid * (1 - person1RepaymentShare);
    ...
    const person1EquityShare: number = totalEquity > 0 ? (person1Equity / totalEquity) * 100 : 0;
    const person2EquityShare: number = totalEquity > 0 ? (person2Equity / totalEquity) * 100 : 0;
  ...
  }, [splitPrincipal, frequency, rate, termYears, selectedYear, person1Deposit, person2Deposit, person1RepaymentShare]);
